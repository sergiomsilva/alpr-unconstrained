    ...
    def info(self, source):
        ...
        try:
            ...
            if format == self.FORMAT_ARRAY:
                ...
                rows, cols = list(map(int, line))
                entries = rows * cols
            else:
                if not len(line) == 3:
                    raise ValueError("Header line not of length 3: " + line)
                rows, cols, entries = list(map(int, line))
            ...
            for line in stream:
                ...
                if entry_number+1 > entries:
                    raise ValueError("'entries' in header is smaller than "
                                     "number of entries")
                l = line.split()
                I[entry_number], J[entry_number] = list(map(int, l[:2]))
                ...
                if not is_pattern:
                    if is_integer:
                        ...
                        V[entry_number] = complex(*list(map(float, l[2:])))
                    else:
