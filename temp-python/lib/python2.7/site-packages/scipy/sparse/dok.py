...
class dok_matrix(spmatrix, IndexMixin, dict):
    ...
    def __getitem__(self, index):
        ...
        if i_intlike and j_intlike:
            ...
            i_seq = range(*i_indices)
            j_seq = range(*j_indices)
    ...
    def _getitem_ranges(self, i_indices, j_indices, shape):
        ...
        i_start, i_stop, i_stride = list(map(int, i_indices))
        j_start, j_stop, j_stride = list(map(int, j_indices))
    ...
    def __setitem__(self, index, x):
        ...
        dict.update(self, zip(zip(i.flat, j.flat), x.flat))

        if 0 in x:
            zeroes = x == 0
            for key in zip(i[zeroes].flat, j[zeroes].flat):
    ...
    def getrow(self, i):
        ...
        dict.update(new, (((0, j), self[i, j]) for j in range(self.shape[1])))
        return new

    def getcol(self, j):
        """Returns the j-th column as a (m x 1) DOK matrix."""
        new = dok_matrix((self.shape[0], 1), dtype=self.dtype)
        dict.update(new, (((i, 0), self[i, j]) for i in range(self.shape[0])))
        return new
